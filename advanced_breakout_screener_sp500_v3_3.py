...
<div class="mh-section">
  ...
  <div style="display:grid;grid-template-columns:auto 1fr auto;gap:20px;align-items:center;margin-bottom:20px;background:var(--glass-bg);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid var(--glass-border);border-radius:10px;padding:16px 20px">
    <!-- Kurs -->
    <div>
    ...
    <!-- Urteil Mini-Gauge -->
    <div style="text-align:center;border-left:1px solid var(--glass-border);padding-left:20px">
...
def build_html(df: pd.DataFrame, spx: dict = None, fgi: dict = None,
    ...
    for i, row in top.iterrows():
        ...
        # Heatmap class based on score (Upgrade 4)
        heat_class = ""
        if row['score'] >= 60:   heat_class = " heat-fire"
        elif row['score'] >= 45: heat_class = " heat-strong"
        elif row['score'] >= 30: heat_class = " heat-mid"
        ...
        <tr{border_s} class="data-row{heat_class}"
          ...
          <td style="padding:10px 14px;font-weight:800;font-size:15px" class="{'score-fire' if row['score']>=60 else 'score-strong' if row['score']>=45 else 'score-mid' if row['score']>=30 else 'score-weak'}">{row['score']:.1f}</td>
        </tr>{news_expand}"""
  ...
  <div class="table-wrap">
    <table id="mainTable">
      <thead><tr>
        ...
        <th onclick="sort(7)">30T PERF<span class="sort-arrow">▲</span></th>
