"""
╔══════════════════════════════════════════════════════════════════╗
║   S&P 500 · ADVANCED BREAKOUT SCREENER  v5.0                    ║
║   Performance · Signalqualität · Robustheit                     ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  NEU in v5.0: PERFORMANCE                                       ║
║  • ThreadPoolExecutor: Downloads laufen parallel (4 Threads)    ║
║  • Tages- & Intraday-Downloads gleichzeitig (nicht sequenziell) ║
║  • Tageskerzen-Cache (Parquet) – wird nur 1×/Tag neu geladen    ║
║  • Analyse-Loop ebenfalls parallelisiert                        ║
║                                                                  ║
║  NEU in v5.0: SIGNALQUALITÄT                                    ║
║  • Earnings-Filter: Aktien ±3T vor/nach Report → Warnung/Flag   ║
║  • Sektor-Stärke: Sektorzugehörigkeit aus Wikipedia             ║
║    Top-Sektoren erhalten Bonus, schwache Sektoren Malus         ║
║  • Relative Stärke vs. SPX: perf_aktie - perf_spx (20T)        ║
║    Outperformer bekommen zusätzliche Scoring-Punkte             ║
║                                                                  ║
║  NEU in v5.0: ROBUSTHEIT                                        ║
║  • Strukturiertes Logging (logging-Modul, Datei + Konsole)      ║
║  • US-Feiertagsliste 2024/2025/2026 – kein Intraday an Feiertag ║
║  • Fehlergrund wird pro Ticker geloggt (nicht mehr still None)  ║
║  • Delta-Score: Vergleich mit letztem CSV-Lauf (▲/▼ Anzeige)   ║
║                                                                  ║
║  v4.0: Hybride Datenpipeline (Tageskerzen + 1h-Intraday)       ║
║  v3.0: S&P500 Marktgesundheits-Analyse + Fear & Greed Index    ║
║  v2.1: Failed-Breakout-Filter + Kombinations-Boni              ║
║                                                                  ║
...
def _setup_logging() -> logging.Logger:
    ...
    # Datei-Handler: alles ab DEBUG
    fh = logging.FileHandler(LOG_FILE, encoding="utf-8")
    fh.setLevel(logging.DEBUG)
    fh.setFormatter(fmt)
    # Konsolen-Handler: nur WARNING und höher (bleibt sauber)
...
# NYSE-Feiertage 2024-2026 (kein Handel an diesen Tagen)
_NYSE_HOLIDAYS = {
...
def _is_trading_day(d: date | None = None) -> bool:
    """True wenn d ein Handelstag ist (Mo-Fr, kein NYSE-Feiertag)."""
    if d is None:
        d = date.today()
...
def analyze_sp500_health() -> dict:
    ...
    try:
        ...
        # ── VIX ──────────────────────────────────────────────────────────
        if not vix_d.empty:
            vix_val = float(vix_d["Close"].squeeze().iloc[-1])
...
<div class="mh-section">
  ...
  <div class="mh-header">Marktgesundheits-Check <span style="font-size:14px;color:#333;font-weight:400">— Stand: {datetime.now().strftime('%d.%m.%Y %H:%M')}</span></div>

  <!-- Preis-Grid -->
  <div class="mh-grid">
    <div class="mh-card">
      <div class="mh-card-label">S&amp;P 500 Kurs</div>
      <div class="mh-card-val">${spx.get('price', 'N/A'):,.2f}</div>
      <div class="mh-card-sub">1T: {color_pct(chg1)} | 5T: {color_pct(chg5)} | 20T: {color_pct(chg20)}</div>
    </div>
    <div class="mh-card">
      <div class="mh-card-label">RSI (14) &amp; MACD</div>
      <div class="mh-card-val">{spx.get('rsi', 'N/A')}</div>
      <div class="mh-card-sub">RSI {spx.get('rsi_trend','')}&nbsp; | MACD {'↑ bullish' if spx.get('macd_bullish') else '↓ bearish'}</div>
    </div>
    <div class="mh-card">
      <div class="mh-card-label">VIX (Volatilität)</div>
      <div class="mh-card-val">{spx.get('vix', 'N/A')}</div>
      <div class="mh-card-sub">{spx.get('vix_label', '')}</div>
    </div>
  </div>
  ...
  <div class="mh-ma-row">
    ...
    <div class="mh-ma-pill">Trend: {spx.get('trend_label','—')}</div>
  </div>

  <!-- Fear & Greed -->
  <div class="mh-fgi-wrap">
    <div style="display:flex;justify-content:space-between;align-items:center">
      <div style="font-size:11px;color:#444;letter-spacing:2px;text-transform:uppercase">Fear &amp; Greed Index (CNN)</div>
      <div style="font-size:22px;font-weight:700;color:{fgi_color}">{fgi_val} — {fgi_label}</div>
    </div>
    <div class="mh-fgi-bar-bg">
    ...
    <div style="font-size:11px;color:#555;margin-top:10px">{fgi_desc}</div>
  </div>
  ...
  <div class="mh-verdict-wrap">
    <div>
      <div style="font-size:11px;color:#444;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px">Markt-Urteil</div>
      ...
      <div style="font-size:9px;color:#333;margin-top:4px">{score:.0f}/100 Technische Stärke</div>
    </div>
...
# v5.0: PERFORMANCE
# ──────────────────────────────────────────────
PARALLEL_WORKERS    = 4       # ThreadPoolExecutor Threads für Downloads & Analyse
CACHE_DIR           = Path(".screener_cache")   # Verzeichnis für Tageskerzen-Cache
CACHE_MAX_AGE_H     = 6       # Cache ungültig nach X Stunden (0 = immer neu laden)

# ──────────────────────────────────────────────
# v5.0: SIGNALQUALITÄT
# ──────────────────────────────────────────────
EARNINGS_WARN_DAYS  = 3       # Tage vor/nach Earnings → Warnung im Report
SECTOR_BONUS        = 8       # Punkte-Bonus für Top-Sektoren
SECTOR_MALUS        = -5      # Punkte-Malus für schwache Sektoren
RS_SPX_BONUS        = 10      # Bonus wenn Aktie S&P500 um >2% outperformt (20T)
RS_SPX_THRESHOLD    = 0.02    # Mindest-Outperformance vs SPX für Bonus
...
MIN_PRICE_TREND_5D      = -0.05  # Preis muss in 5T > -5% sein (grobe Absicherung)
...
WEIGHTS = {
    ...
    "ma60_crossover_recent":   25,   # Crossover in letzten 1-3 Tagen
    "price_above_ma60_rising": 15,   # Über MA60 + MA60 selbst steigt

    # ── BREAKOUT-GESUNDHEIT ──────────────────────────────────────
    "breakout_holding":        35,   # Nach Cross: Preis hält das Cross-Niveau
    "breakout_continuing":     20,   # Nach Cross: Preis steigt jeden Tag weiter

    # ── PREIS-STRUKTUR ───────────────────────────────────────────
    "close_near_day_high":     20,   # Schlusskurs > 70% der Tagesrange
    "higher_highs_lows":       20,   # Höhere Hochs UND höhere Tiefs
    "breakout_of_range":       25,   # Ausbruch aus mehrtägiger enger Range

    # ── MOMENTUM ────────────────────────────────────────────────
    "rsi_rising_above_50":     25,   # RSI > 50 UND steigt
    "rsi_ideal_zone":          15,   # RSI 55–70 (ideal für Breakout-Continuation)
    "macd_fresh_crossover":    25,   # MACD kreuzt Signal nach oben (max 2T alt)
    ...
    # ── RELATIVE STÄRKE (inkl. neu: vs SPX) ─────────────────────
    ...
    "rs_vs_spx":               10,   # NEU v5: Outperformt S&P500 (20T, >2%)
}
...
COMBO_BONUSES = [
    ...
    (["rs_vs_spx", "ma60_crossover_today"],                                        15, "🏆 SPX LEADER"),  # NEU v5
]


# ══════════════════════════════════════════════
# SCHRITT 1: TICKER LADEN + SEKTORDATEN (v5.0)
# ══════════════════════════════════════════════

# Globale Sektorzuordnung: {ticker: sektor}
SECTOR_MAP: dict[str, str] = {}

# Sektoren die aktuell tendenziell stark/schwach sind
# (wird in analyze_sp500_health() befüllt, sonst leer → kein Bonus/Malus)
STRONG_SECTORS: set[str] = set()
WEAK_SECTORS:   set[str] = set()


def get_tickers() -> list:
    global SECTOR_MAP
    print("\n📋 Lade S&P 500 Ticker-Liste + Sektordaten...")
    tickers, sector_map = _try_wikipedia_with_sectors()
    if not tickers:
        tickers = _try_ishares_ivv() or _fallback_list()
    SECTOR_MAP = sector_map
    sector_count = len([v for v in sector_map.values() if v])
    print(f"   ✓ {len(tickers)} Ticker | {sector_count} mit Sektor-Info")
    log.info(f"Ticker geladen: {len(tickers)}, Sektordaten: {sector_count}")
    return tickers


def _try_wikipedia_with_sectors() -> tuple[list, dict]:
    """
    S&P 500 Komponenten von Wikipedia inkl. GICS-Sektor (Spalte 3).
    Gibt (tickers, {ticker: sektor}) zurück.
    """
    try:
        ...
        for row in table.find_all("tr")[1:]:
            ...
            if cols:
                ...
                # Spalte 3 = GICS Sector
                sector = cols[3].text.strip() if len(cols) > 3 else ""
                if t:
                    tickers.append(t)
                    sector_map[t] = sector
        if len(tickers) > 400:
        ...
        log.warning(f"Wikipedia-Abruf fehlgeschlagen: {e}")
        return [], {}


def _try_wikipedia() -> list:
    tickers, _ = _try_wikipedia_with_sectors()
    return tickers
...
# ══════════════════════════════════════════════════════════════════
# SCHRITT 2: KURSDATEN LADEN (HYBRID + CACHE + PARALLEL v5.0)
# ══════════════════════════════════════════════════════════════════

def _is_market_open() -> bool:
    """
    Prüft ob NYSE gerade geöffnet ist.
    Berücksichtigt Wochenenden und NYSE-Feiertage (v5.0).
    """
    today = date.today()
    if not _is_trading_day(today):
        return False
    # UTC-5 für ET (Winter) / UTC-4 (Sommer) – vereinfacht auf UTC-5
...
def _cache_valid(path: Path) -> bool:
    if CACHE_MAX_AGE_H <= 0 or not path.exists():
        return False
...
def _cache_load(key: str):
    ...
    if _cache_valid(p):
        try:
            ...
            log.warning(f"Cache-Lesefehler {key}: {e}")
    return None
...
def _cache_save(key: str, obj) -> None:
    try:
        ...
        log.warning(f"Cache-Schreibfehler {key}: {e}")


# ── Intraday-Aggregation ────────────────────────────────────────────

def _aggregate_intraday_to_day(intraday_1h: pd.DataFrame) -> pd.DataFrame:
    """Aggregiert 1h-Kerzen zu Tageskerzen (OHLCV)."""
    df = intraday_1h.copy()
...
def _normalize_index(df: pd.DataFrame) -> pd.DataFrame:
    """Normalisiert den DatetimeIndex: tz-naiv, auf Datum gerundet, dedupliziert."""
    df = df.copy()
...
def _build_hybrid_df(daily: pd.DataFrame, intraday_1h: pd.DataFrame | None) -> pd.DataFrame:
    """
    Ersetzt die letzten 5 Tage der Tageskerzen durch aggregierte 1h-Kerzen.
    Letzter Datenpunkt = aktueller Intraday-Stand (auch während Börsenzeit).
    """
    daily = _normalize_index(daily)

    if intraday_1h is None or intraday_1h.empty:
        return daily

    intra_daily = _normalize_index(_aggregate_intraday_to_day(intraday_1h))

    cutoff   = intra_daily.index.min()
    hist     = daily[daily.index < cutoff]
    combined = pd.concat([hist, intra_daily])
...
# ── Batch-Download-Helfer ───────────────────────────────────────────

def _extract_ticker_df(raw: pd.DataFrame, ticker: str, is_single: bool) -> pd.DataFrame | None:
    """
    Extrahiert einen einzelnen Ticker-DataFrame aus einem yf.download()-Ergebnis.
    Unterstützt beide yfinance-Formate:
      - Alt (< 0.2.x): raw[ticker] → DataFrame mit OHLCV-Spalten
      - Neu (≥ 0.2.x): MultiIndex (Price, Ticker) → raw.xs(ticker, level=1, axis=1)
    """
    if is_single:
        # Einzelner Ticker: direkt das ganze DataFrame
        df = raw.copy()
        # Neueres yfinance gibt manchmal MultiIndex auch bei Einzel-Ticker
    ...
    # Mehrere Ticker
    try:
        # Versuch 1: direkter Zugriff (ältere yfinance-Versionen)
        if ticker in raw.columns and not isinstance(raw.columns, pd.MultiIndex):
            return raw[[ticker]].copy()

        # Versuch 2: MultiIndex Level 0 = Preis-Felder, Level 1 = Ticker (neueres yfinance)
        ...
        # Versuch 3: MultiIndex Level 0 = Ticker (älteres group_by="ticker")
        if isinstance(raw.columns, pd.MultiIndex):
...
def _download_daily_batch(batch: list, start_str: str, end_str: str) -> dict:
    """
    Lädt Tageskerzen für eine Ticker-Liste.
    Strategie: Batch zuerst (schnell), dann Einzel-Retry für fehlende Ticker.
    Unterstützt altes und neues yfinance MultiIndex-Format.
    """
    result = {}
    ...
    try:
        ...
        for t in batch:
            try:
                df = _extract_ticker_df(raw, t, is_single)
                if df is None:
                    continue
        ...
        log.warning(f"Daily-Batch fehlgeschlagen: {e}")

    # Versuch 2: Einzel-Retry für fehlende Ticker
    ...
    for t in missing:
        try:
            ...
            if len(df) >= 80:
                ...
                log.debug(f"{t}: Zu wenig Tageskerzen nach Retry ({len(df)})")
        except Exception as e:
            log.debug(f"{t}: Einzel-Download fehlgeschlagen: {e}")

    return result


def _download_intraday_batch(batch: list) -> dict:
    """
    Lädt 1h-Intraday-Daten für eine Ticker-Liste.
    Strategie: Batch zuerst, Einzel-Retry für fehlende Ticker.
    Unterstützt altes und neues yfinance MultiIndex-Format.
    """
    result = {}
    is_single = len(batch) == 1
    ...
    try:
        ...
        log.warning(f"Intraday-Batch fehlgeschlagen: {e}")

    # Versuch 2: Einzel-Retry
    missing = [t for t in batch if t not in result]
    for t in missing:
        try:
            df = yf.download(t, period="5d", interval="1h",
                             auto_adjust=True, progress=False)
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)
            df = df.dropna(subset=["Close"])
            ...
            log.debug(f"{t}: Intraday-Einzel-Download fehlgeschlagen: {e}")

    return result  # war vergessen!
...
def download_prices(tickers: list) -> dict:
    """
    Hybride Datenpipeline v5.0:
    • Tageskerzen: Cache-first (max CACHE_MAX_AGE_H Stunden alt)
      → parallelisierte Batches via ThreadPoolExecutor
    • Intraday 1h: immer frisch, parallel zu Tages-Phase
    • Merge: letzte 5 Tage durch Intraday-Tageskerzen ersetzt
    • Feiertags-Check: kein Intraday an NYSE-Feiertagen
    """
    if MAX_TICKERS:
    ...
    is_holiday   = not _is_trading_day()
    fetch_intra  = market_open  # Intraday NUR wenn Börse gerade geöffnet ist

    status = ("🟢 BÖRSE GEÖFFNET – Intraday aktiv" if market_open else
              "🏖 FEIERTAG – nur Tageskerzen"       if is_holiday  else
              "🔴 Börse geschlossen – letzter Tagesschluss")

    print(f"\n📥 Lade Kursdaten für {len(tickers)} Ticker | {status}")
    print(f"   Threads: {PARALLEL_WORKERS} | Cache: {'aktiv' if CACHE_MAX_AGE_H > 0 else 'deaktiviert'}")
    log.info(f"Download-Start: {len(tickers)} Ticker, market_open={market_open}, holiday={is_holiday}")

    # ── Phase 1: Tageskerzen (Cache + parallele Batches) ────────────
    cache_key  = f"daily_{date.today().isoformat()}_{len(tickers)}"
    daily_data = _cache_load(cache_key)

    # Cache-Validierung: ablehnen wenn weniger als 80% der Ticker enthalten
    expected_min = int(len(tickers) * 0.8)
    if daily_data is not None and len(daily_data) < expected_min:
        print(f"   ⚠  Cache unvollständig ({len(daily_data)}/{len(tickers)} Ticker) → neu laden")
        log.warning(f"Cache verworfen: nur {len(daily_data)} Ticker (Minimum: {expected_min})")
        daily_data = None

    if daily_data is not None:
        print(f"   ✓ Tageskerzen aus Cache ({len(daily_data)} Ticker)")
        log.info(f"Cache-Hit: {len(daily_data)} Ticker")
    else:
        print(f"   Phase 1/2: Lade Tageskerzen parallel ({PARALLEL_WORKERS} Threads)...")
        batches    = [tickers[i:i+BATCH_SIZE] for i in range(0, len(tickers), BATCH_SIZE)]
        daily_data = {}

        with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as pool:
            futures = {
                pool.submit(_download_daily_batch, b, start_str, end_str): b
                for b in batches
            }
            for fut in tqdm(as_completed(futures), total=len(futures), desc="  Daily"):
                try:
                    daily_data.update(fut.result())
                except Exception as e:
                    log.warning(f"Daily-Future-Fehler: {e}")

        _cache_save(cache_key, daily_data)
        print(f"   ✓ {len(daily_data)} Ticker mit Tageskerzen (Cache gespeichert)")
        log.info(f"Daily geladen: {len(daily_data)}")

    # ── Phase 2: Intraday 1h (parallel, falls Handelstag) ──────────
    intraday_data = {}
    if fetch_intra and not is_holiday:
        valid_tickers = list(daily_data.keys())
        intra_batches = [valid_tickers[i:i+BATCH_SIZE] for i in range(0, len(valid_tickers), BATCH_SIZE)]
        print(f"   Phase 2/2: Lade 1h-Intraday parallel ({PARALLEL_WORKERS} Threads)...")

        with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as pool:
            futures = {
                pool.submit(_download_intraday_batch, b): b
                for b in intra_batches
            }
            for fut in tqdm(as_completed(futures), total=len(futures), desc="  Intraday"):
                try:
                    intraday_data.update(fut.result())
                except Exception as e:
                    log.warning(f"Intraday-Future-Fehler: {e}")

        log.info(f"Intraday geladen: {len(intraday_data)}")
    else:
        reason = "Feiertag" if is_holiday else "kein Handelstag"
        print(f"   Phase 2/2: Intraday übersprungen ({reason})")

    # ── Phase 3: Merge ───────────────────────────────────────────────
    data = {}
    merged_count = 0
    for t, daily_df in daily_data.items():
        intra_df = intraday_data.get(t)
        try:
            hybrid = _build_hybrid_df(daily_df, intra_df)
            if len(hybrid) >= 80:
                data[t] = hybrid
                if intra_df is not None and not intra_df.empty:
                    merged_count += 1
            else:
                log.debug(f"{t}: Nach Merge zu wenig Zeilen ({len(hybrid)})")
        except Exception as e:
            log.warning(f"{t}: Merge-Fehler: {e}")
            if len(daily_df) >= 80:
                data[t] = daily_df
    ...
    if intraday_data:
        try:
            sample = next(iter(intraday_data.values()))
            intra_time_str = pd.Timestamp(sample.index[-1]).strftime("%H:%M")
        except Exception:
            pass

    print(f"   ✓ {len(data)} Ticker gesamt | {merged_count} mit Intraday | Stand: {intra_time_str} ET")
    log.info(f"Merge abgeschlossen: {len(data)} Ticker, {merged_count} Intraday-Merged")
    return data
...
# ══════════════════════════════════════════════════════════════════
# SIGNALQUALITÄT: EARNINGS + SEKTOR + RS vs SPX  (v5.0)
# ══════════════════════════════════════════════════════════════════

# Cache für Earnings-Daten: {ticker: (next_earnings_date | None, fetched_ts)}
_earnings_cache: dict[str, tuple] = {}


def get_earnings_info(ticker: str) -> dict:
    """
    Gibt Earnings-Info für einen Ticker zurück:
      days_to_earnings: int | None  (negativ = bereits vorbei, None = unbekannt)
      earnings_warning: bool         (True wenn innerhalb ±EARNINGS_WARN_DAYS)
      earnings_date:    str | None
    """
    now = datetime.now()

    # In-Memory-Cache (gültig 1h)
    if ticker in _earnings_cache:
        dt, fetched_ts = _earnings_cache[ticker]
        if (now - fetched_ts).total_seconds() < 3600:
            return _build_earnings_result(dt, now)

    try:
        info = yf.Ticker(ticker).calendar
        if info is None or info.empty:
            _earnings_cache[ticker] = (None, now)
            return _build_earnings_result(None, now)

        # yfinance liefert ein DataFrame mit Earnings Date als Spalte oder Index
        if "Earnings Date" in info.columns:
            raw = info["Earnings Date"].iloc[0]
        elif "Earnings Date" in info.index:
            raw = info.loc["Earnings Date"].iloc[0]
        else:
            raw = None
        ...
        _earnings_cache[ticker] = (dt, now)
        return _build_earnings_result(dt, now)
    except Exception as e:
        log.debug(f"{ticker}: Earnings-Abruf fehlgeschlagen: {e}")
        _earnings_cache[ticker] = (None, now)
        return _build_earnings_result(None, now)


def _build_earnings_result(dt: datetime | None, now: datetime) -> dict:
    if dt is None:
        return {"days_to_earnings": None, "earnings_warning": False, "earnings_date": None}
    days = (dt.date() - now.date()).days
    warn = abs(days) <= EARNINGS_WARN_DAYS
    return {
        "days_to_earnings": days,
        "earnings_warning": warn,
        "earnings_date":    dt.strftime("%Y-%m-%d"),
    }


def get_sector_adjustment(ticker: str) -> int:
    """
    Gibt Sektor-Bonus (+) oder Malus (-) in Rohpunkten zurück.
    Basis: STRONG_SECTORS / WEAK_SECTORS (befüllt durch analyze_sp500_health).
    """
...
# Globale SPX-20T-Performance (wird in main() gesetzt)
SPX_PERF_20D: float | None = None


def _get_spx_perf_20d() -> float | None:
    """Lädt die 20-Tage-Performance des S&P 500 (gecacht in SPX_PERF_20D)."""
    global SPX_PERF_20D
    if SPX_PERF_20D is not None:
        return SPX_PERF_20D
    try:
        spx = yf.download("^GSPC", period="30d", progress=False, auto_adjust=True)
        if not spx.empty and len(spx) >= 20:
            c = spx["Close"].squeeze()
            SPX_PERF_20D = float((c.iloc[-1] - c.iloc[-20]) / c.iloc[-20])
            return SPX_PERF_20D
    except Exception as e:
        log.debug(f"SPX-20T-Performance: {e}")
    return None
...
def pre_filter(ticker: str, df: pd.DataFrame) -> tuple[bool, str]:
    ...
    v5.1: Adaptiv – wenn der S&P 500 selbst in einer Korrektur ist
    (SPX_PERF_20D < -3%), werden die Trend-Schwellwerte automatisch
    gelockert, damit der Screener auch in schwachen Marktphasen
    ...
    # Adaptive Schwellwerte: in Korrektur gelockert
    # Trigger bereits bei -1% SPX 20T-Performance (nicht erst bei -3%)
    spx_correction = (SPX_PERF_20D is not None and SPX_PERF_20D < -0.01)
    trend_20d_min  = MIN_PRICE_TREND_20D * 2.0 if spx_correction else MIN_PRICE_TREND_20D
    trend_5d_min   = MIN_PRICE_TREND_5D  * 2.0 if spx_correction else MIN_PRICE_TREND_5D
    ma60_dist_min  = MAX_DIST_BELOW_MA60 * 1.5 if spx_correction else MAX_DIST_BELOW_MA60
    ...
    if n >= 65:
        ...
        for lookback in range(2, 6):
            ...
            if was_cross:
                drawdown_since = (today_c - c_at) / c_at
...
def analyze(ticker: str, df: pd.DataFrame) -> dict | None:
    try:
        ...
        # ── NEU v5: RELATIVE STÄRKE vs. S&P500 ───────────────────
        spx_perf = _get_spx_perf_20d()
        perf_20d_val = (
            (today_c - float(close.iloc[-20])) / float(close.iloc[-20])
            if n >= 20 else None
        )
        rs_vs_spx = (
            perf_20d_val is not None and
            spx_perf is not None and
            (perf_20d_val - spx_perf) > RS_SPX_THRESHOLD
        )
        rs_outperf_pct = (
            round((perf_20d_val - spx_perf) * 100, 1)
            if (perf_20d_val is not None and spx_perf is not None) else None
        )

        # ── NEU v5: EARNINGS-INFO ─────────────────────────────────
        earnings = get_earnings_info(ticker)

        # ── NEU v5: SEKTOR-ADJUSTMENT ─────────────────────────────
        sector_adj = get_sector_adjustment(ticker)
        sector     = SECTOR_MAP.get(ticker, "")
        ...
        add("rs_vs_spx",               rs_vs_spx)   # NEU v5

        # ── SEKTOR-ADJUSTMENT (Rohpunkte, nicht normalisiert) ──────
        score += sector_adj
        ...
        # ── INTRADAY-ERKENNUNG ────────────────────────────────────
        # Wenn der letzte Index-Eintrag dem heutigen Datum entspricht
        # UND der Index eine Uhrzeit enthält (tz-aware oder nicht),
        # wissen wir dass wir Intraday-Daten haben.
        last_idx      = df.index[-1]
        last_date_str = pd.Timestamp(last_idx).strftime("%Y-%m-%d")
        today_str     = datetime.today().strftime("%Y-%m-%d")
        is_intraday   = (last_date_str == today_str)
        # Zeitstempel für Anzeige
        try:
            last_time_str = pd.Timestamp(last_idx).strftime("%H:%M")
        except Exception:
            last_time_str = ""
        data_label = (f"LIVE {last_time_str} ET" if is_intraday else last_date_str)
        ...
        return {
            ...
            # NEU v5: Signalqualität
            "sector":              sector,
            "sector_adj":          sector_adj,
            "rs_vs_spx":           rs_vs_spx,
            "rs_outperf_pct":      rs_outperf_pct,
            "earnings_warning":    earnings["earnings_warning"],
        ...
        log.debug(f"{ticker}: analyze() Fehler: {e}")
        return None


def _load_prev_scores(csv_path: str) -> dict[str, float]:
    """Lädt Score-Werte aus dem letzten CSV-Lauf für Delta-Anzeige."""
    try:
        if not Path(csv_path).exists():
            return {}
        prev = pd.read_csv(csv_path, usecols=["ticker", "score"])
        return dict(zip(prev["ticker"], prev["score"]))
    except Exception as e:
        log.debug(f"Vorheriges CSV nicht lesbar: {e}")
        return {}


def analyze_all(price_data: dict) -> pd.DataFrame:
    """
    v5.0: Parallelisierte Analyse mit ThreadPoolExecutor.
    Fügt Delta-Score (Vergleich mit letztem CSV-Lauf) ein.
    Gibt Filtergrund-Statistik aus für einfacheres Debugging.
    """
    prev_scores = _load_prev_scores(OUTPUT_CSV)
    n = len(price_data)
    print(f"\n🔬 Analysiere {n} Ticker parallel ({PARALLEL_WORKERS} Threads)...")

    # Sicherstellen dass SPX_PERF_20D geladen ist (Fallback falls main() es nicht gesetzt hat)
    if SPX_PERF_20D is None:
        _get_spx_perf_20d()

    # Korrektur-Status anzeigen
    spx_correction = (SPX_PERF_20D is not None and SPX_PERF_20D < -0.01)
    if spx_correction:
        print(f"   ⚡ Adaptiver Modus: SPX 20T={SPX_PERF_20D*100:.1f}% → Vorfilter gelockert")
    elif SPX_PERF_20D is not None:
        print(f"   ℹ  SPX 20T: {SPX_PERF_20D*100:.1f}% (Normalmodus, kein adaptiver Filter)")
    else:
        print(f"   ℹ  SPX-Performance nicht verfügbar → Standard-Schwellwerte aktiv")

    log.info(f"Analyse-Start: {n} Ticker, SPX_PERF_20D={SPX_PERF_20D}, adaptiver_modus={spx_correction}")

    # Für Debugging: pre_filter direkt aufrufen und Gründe sammeln
    filter_reasons: dict[str, int] = {}
    passed = []
    failed = []

    # Erst Vorfilter sequenziell (schnell, gibt uns Debugging-Info)
    ...
    # Filtergrund-Statistik
    if failed:
        print(f"   📊 Vorfilter: {len(passed)} bestanden, {len(failed)} herausgefiltert")
        for reason, count in sorted(filter_reasons.items(), key=lambda x: -x[1])[:5]:
            print(f"      · {count:>3}× {reason}")
        log.info(f"Vorfilter-Gründe: {filter_reasons}")
    else:
        print(f"   ✓ Alle {len(passed)} Ticker durch Vorfilter")

    if not passed:
        return pd.DataFrame()

    # Jetzt nur die durchgekommenen Ticker parallel analysieren
    results = []
    ...
    with ThreadPoolExecutor(max_workers=PARALLEL_WORKERS) as pool:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures), desc="Analysiere"):
            ...
            try:
                ...
                log.warning(f"{t}: Analyse-Future-Fehler: {e}")

    df = pd.DataFrame(results)
    if df.empty:
        return df

    # Delta-Score berechnen
    df["score_delta"] = df.apply(
        lambda row: round(row["score"] - prev_scores[row["ticker"]], 1)
        if row["ticker"] in prev_scores else None,
        axis=1
    )

    df = df.sort_values("score", ascending=False).reset_index(drop=True)
    log.info(f"Analyse abgeschlossen: {len(df)} Aktien")
    print(f"   ✓ {len(df)} Aktien im Ergebnis")
    if prev_scores:
        print(f"   📊 Score-Delta verfügbar (Vergleich mit letztem Lauf)")
    return df
...
def build_html(df: pd.DataFrame, spx: dict = None, fgi: dict = None) -> str:
    top     = df.head(150)
    ...
    for i, row in top.iterrows():
        ...
        data_label  = row.get("data_label", row.get("last_date", ""))
        live_badge  = (
            f'<span style="background:rgba(0,255,136,0.15);color:#00ff88;'
            f'border:1px solid rgba(0,255,136,0.4);font-size:9px;padding:1px 6px;'
            f'border-radius:3px;margin-left:4px;letter-spacing:1px">⚡ {data_label}</span>'
            if is_intraday else
            f'<span style="color:#333;font-size:9px;margin-left:4px">{data_label}</span>'
        )

        # Earnings-Badge
        earn_warn = row.get("earnings_warning", False)
        earn_days = row.get("days_to_earnings")
        earn_date = row.get("earnings_date", "")
        if earn_warn and earn_days is not None:
            earn_dir  = "in" if earn_days >= 0 else "vor"
            earn_abs  = abs(earn_days)
            earn_badge = (
                f'<span title="Earnings {earn_date}" style="background:rgba(255,200,0,0.15);'
                f'color:#ffd600;border:1px solid rgba(255,200,0,0.4);font-size:9px;'
                f'padding:1px 6px;border-radius:3px;margin-left:4px">⚠ EARNINGS {earn_dir} {earn_abs}T</span>'
            )
        else:
            earn_badge = ""
        ...
        if sector:
            sec_col = "#69f0ae" if sec_adj > 0 else "#ff6d6d" if sec_adj < 0 else "#555"
            sec_pill = (
                f'<div style="font-size:9px;color:{sec_col};margin-top:3px;'
                f'letter-spacing:0.5px">{sector[:22]}'
                f'{"  ▲" if sec_adj > 0 else "  ▼" if sec_adj < 0 else ""}</div>'
            )
        else:
            sec_pill = ""

        # RS vs SPX
        rs_pct  = row.get("rs_outperf_pct")
        rs_pill = (
            f'<span style="font-size:9px;color:#69f0ae;margin-left:4px">'
            f'RS +{rs_pct:.1f}%</span>'
            if (rs_pct is not None and rs_pct > 0) else ""
        )

        # Score-Delta
        delta     = row.get("score_delta")
        if delta is not None:
            dc = "#69f0ae" if delta > 0 else "#ff6d6d" if delta < 0 else "#555"
            ds = "▲" if delta > 0 else "▼" if delta < 0 else "–"
            delta_html = f'<span style="font-size:10px;color:{dc};margin-left:4px">{ds}{abs(delta):.1f}</span>'
        else:
            delta_html = ""

        score_color = ('#00ff88' if row['score']>=60 else '#ffaa00' if row['score']>=45
                       else '#ff9944' if row['score']>=30 else '#ff4466')
        ...
        <tr{border_s} class="data-row"
          ...
          <td style="padding:10px 14px;font-weight:900;font-size:14px;color:#fff;font-family:'Courier New',monospace">
            {row['ticker']}{live_badge}{earn_badge}
            {sec_pill}
          </td>
          <td style="padding:10px 14px">{badge(row['signal'])}</td>
          <td style="padding:10px 14px">{prob_bar(row['score'])}</td>
          <td style="padding:10px 14px;color:#fff;font-weight:700">${row['price']:.2f}{rs_pill}</td>
          ...
          <td style="padding:10px 14px;font-weight:800;font-size:15px;color:{score_color}">
            {row['score']:.1f}{delta_html}
          </td>
        </tr>"""
...
<title>Breakout Screener v5.0 · S&P 500</title>
...
<div class="wrap">
  <header>
    ...
    <h1>Breakout <em>Intelligence</em> <span class="version">v5.0</span></h1>
  ...
  <div class="footer">
    // S&P 500 Advanced Breakout Screener v5.0 · Daten: Yahoo Finance via yfinance<br>
    // Parallel-Downloads · Tageskerzen-Cache · Earnings-Filter · Sektor-Stärke · RS vs SPX · Score-Delta · Nicht als Anlageberatung zu verstehen
  </div>
</div>
<script>
let curF = 'all';
...
def main():
    print("╔" + "═"*60 + "╗")
    print("║  ADVANCED BREAKOUT SCREENER v5.0 · S&P 500" + " "*17 + "║")
    print("║  Performance · Signalqualität · Robustheit" + " "*17 + "║")
    print("╚" + "═"*60 + "╝")
    log.info("=== Screener v5.0 gestartet ===")

    # ── SCHRITT 0: S&P 500 MARKTANALYSE ────────────────────────────
    ...
    # NEU v5: Sektor-Stärke aus SPX-Analyse ableiten
    # Einfache Heuristik: Bärische Signale-Sektoren → WEAK, sonst neutral
    # In echtem Einsatz: Sektor-ETF-Performance laden (XLK, XLF etc.)
    global STRONG_SECTORS, WEAK_SECTORS, SPX_PERF_20D
    spx_trend = spx_data.get("change_20d_pct", 0) or 0
    if spx_trend > 3:
        # Breiter Aufwärtstrend: zyklische Sektoren bevorzugt
        STRONG_SECTORS = {"Information Technology", "Consumer Discretionary", "Industrials", "Financials"}
        WEAK_SECTORS   = {"Utilities", "Consumer Staples"}
    elif spx_trend < -3:
        # Korrektur: defensive Sektoren bevorzugt
        STRONG_SECTORS = {"Health Care", "Utilities", "Consumer Staples"}
        WEAK_SECTORS   = {"Information Technology", "Consumer Discretionary", "Energy"}
    else:
        STRONG_SECTORS = set()
        WEAK_SECTORS   = set()

    if STRONG_SECTORS:
        print(f"  📈 Starke Sektoren: {', '.join(STRONG_SECTORS)}")
    if WEAK_SECTORS:
        print(f"  📉 Schwache Sektoren: {', '.join(WEAK_SECTORS)}\n")

    # SPX-20T-Performance vorausladen (wird in pre_filter() und analyze() genutzt)
    _get_spx_perf_20d()
    log.info(f"SPX 20T-Performance: {SPX_PERF_20D}")

    # ── SCHRITT 1-3: AKTIEN-SCREENER ───────────────────────────────
    tickers    = get_tickers()

    # Veralteten Cache löschen (z.B. nach Update der Download-Logik)
    # Cache wird nur genutzt wenn er nicht älter als CACHE_MAX_AGE_H ist
    cache_key  = f"daily_{date.today().isoformat()}_{len(tickers) if not MAX_TICKERS else min(len(tickers), MAX_TICKERS)}"
    cache_file = _cache_path(cache_key)
    if cache_file.exists():
        age_h = (time.time() - cache_file.stat().st_mtime) / 3600
        print(f"   💾 Cache: {cache_file.name} ({age_h:.1f}h alt)")
    ...
    if len(df_results) == 0:
        ...
        log.warning("Keine Ergebnisse nach Vorfilter.")
        return

    # CSV (v5: erweiterte Spalten)
    csv_cols = [
        "ticker", "signal", "category", "score", "score_delta", "combo_score", "active_combos",
        "price", "ma60", "pct_from_ma60", "rsi", "macd_hist", "adx", "vol_ratio",
        "bb_squeeze", "obv_new_high", "near_52w_high", "consolidation", "perf_30d",
        "sector", "sector_adj", "rs_vs_spx", "rs_outperf_pct",
        "earnings_warning", "earnings_date", "days_to_earnings",
        "last_date", "is_intraday", "data_label",
    ]
    df_results[[c for c in csv_cols if c in df_results.columns]].to_csv(OUTPUT_CSV, index=False)
    print(f"\n💾 CSV: {OUTPUT_CSV}")
    log.info(f"CSV gespeichert: {OUTPUT_CSV}")
    ...
    log.info(f"HTML gespeichert: {OUTPUT_HTML}")

    # Konsolen-Ausgabe Top N
    n_live    = int(df_results["is_intraday"].sum()) if "is_intraday" in df_results.columns else 0
    n_earn    = int(df_results.get("earnings_warning", pd.Series(False)).sum())
    n_rs      = int(df_results.get("rs_vs_spx", pd.Series(False)).sum())
    has_delta = "score_delta" in df_results.columns and df_results["score_delta"].notna().any()

    print(f"\n{'═'*80}")
    print(f"  🏆 TOP {TOP_N_CONSOLE} BREAKOUT-KANDIDATEN  (Score 0–100)")
    print(f"  ⚡ {n_live} Live-Intraday  |  ⚠ {n_earn} Earnings-Warnung  |  🏆 {n_rs} SPX-Outperformer")
    print(f"{'═'*80}")
    print(f"  {'':2}{'TICKER':<9} {'SIGNAL':<14} {'SCORE':>6}  {'KURS':>7}  "
          f"{'RSI':>5}  {'VOL×':>5}  {'SEKTOR':<22}  KOMBOS")
    print(f"  {'─'*9} {'─'*14} {'─'*6}  {'─'*7}  {'─'*5}  {'─'*5}  {'─'*22}  {'─'*20}")
    ...
    for _, r in df_results.head(TOP_N_CONSOLE).iterrows():
        ...
        live_tag  = "⚡" if r.get("is_intraday") else "  "
        earn_tag  = "⚠" if r.get("earnings_warning") else " "
        rs_tag    = "🏆" if r.get("rs_vs_spx") else "  "
        sector_s  = (r.get("sector") or "")[:20]
        delta     = r.get("score_delta")
        delta_s   = f" ▲{delta:.1f}" if (delta and delta > 0) else (f" ▼{abs(delta):.1f}" if (delta and delta < 0) else "")
        print(f"  {live_tag}{earn_tag}{rs_tag}{r['ticker']:<7} {sig_s:<14} "
              f"{r['score']:>5.1f}{delta_s:<6}  ${r['price']:>6.2f}  "
              f"{r['rsi']:>5.0f}  {r['vol_ratio']:>4.1f}×  {sector_s:<22}  {cat_icon} {combos}")

    print(f"\n  ℹ  ⚡=Intraday  ⚠=Earnings±3T  🏆=SPX-Outperformer  ▲▼=Score-Delta vs. letztem Lauf")
    print(f"  ℹ  Score ≥60=Sehr stark · ≥45=Stark · ≥30=Mittel")
    print(f"  ℹ  Log-Datei: {os.path.abspath(LOG_FILE)}\n")
    log.info("=== Screener v5.0 abgeschlossen ===")
