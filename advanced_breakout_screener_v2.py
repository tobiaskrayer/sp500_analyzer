"""
╔══════════════════════════════════════════════════════════════════╗
║   RUSSELL 2000 · ADVANCED BREAKOUT SCREENER  v2.0               ║
║   Fokus: Nur echte Aufwärts-Breakouts mit starkem Momentum      ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║  KERNPRINZIP v2.0:                                               ║
║  • Harte Vorfilter eliminieren schwache Aktien sofort            ║
║  • Scoring belohnt NUR frisches, bestätigtes Momentum           ║
║  • Kombinationssignale (z.B. Squeeze + Volumen + RSI) extra      ║
║                                                                  ║
║  INDIKATOREN:                                                    ║
║  1. Trend-Qualität     (MA60 Crossover + Bestätigung)           ║
║  2. Preisstruktur      (Höhere Hochs, Nähe zum Tageshoch)        ║
║  3. Momentum           (RSI-Anstieg, MACD-Kreuzung)             ║
║  4. Volumen-Qualität   (Spike bei steigendem Preis)              ║
║  5. Squeeze-Breakout   (BB-Squeeze + Ausbruch kombiniert)       ║
║  6. Relative Stärke    (besser als eigener Schnitt)              ║
║  7. Institutionelles   (OBV steigt + Volumen-Trend)             ║
...
MIN_PRICE_TREND_20D   = -0.03    # Preis muss in 20T mind. -3% sein (kein Absturz)
...
WEIGHTS = {
    # ── TREND-SIGNAL (Kern) ──────────────────────────────────────
    # Frischer MA60-Crossover ist das stärkste Einzelsignal
    "ma60_crossover_today":    60,   # Crossover genau HEUTE → Primärsignal
    "ma60_crossover_recent":   35,   # Crossover in letzten 1-3 Tagen
    "price_above_ma60_rising": 15,   # Über MA60 + MA60 selbst steigt (nachhaltiger Trend)

    # ── PREIS-STRUKTUR ───────────────────────────────────────────
    # Kerzenstruktur und Preisbewegung am Tag des Signals
    "close_near_day_high":     20,   # Schlusskurs > 90% der Tagesrange (kein Reversal!)
    "higher_highs_lows":       20,   # Höhere Hochs UND höhere Tiefs (Aufwärtsstruktur)
    "breakout_of_range":       25,   # Ausbruch aus mehrtägiger enger Range (Kompression → Expansion)

    # ── MOMENTUM ────────────────────────────────────────────────
    "rsi_rising_above_50":     25,   # RSI > 50 UND steigt (nicht nur Niveau)
    "rsi_ideal_zone":          15,   # RSI 55–70 (ideal für Breakout-Continuation)
    "macd_fresh_crossover":    25,   # MACD kreuzt Signal nach oben (frisch = max 2 Tage alt)
    "macd_hist_accelerating":  15,   # MACD-Histogramm steigt mehrere Tage hintereinander

    # ── VOLUMEN-QUALITÄT ─────────────────────────────────────────
    # Volumen nur wertvoll wenn der Preis dabei auch steigt!
    "volume_on_up_day":        30,   # Volumen-Spike (>150%) an einem Up-Day (Bestätigung!)
    "volume_dry_up_before":    15,   # Volumen vorher trocken (Konsolidierung), jetzt explodiert
    "obv_new_high":            20,   # OBV auf neuem 30T-Hoch (Institutionen akkumulieren)

    # ── SQUEEZE-BREAKOUT ─────────────────────────────────────────
    # BB-Squeeze allein reicht nicht – Preis muss sich bereits bewegen
    ...
    # ── RELATIVE STÄRKE ──────────────────────────────────────────
    "outperforming_ma20":      15,   # Heute stärker als 20T-Durchschnitt der letzten Tage
    "near_52w_high":           15,   # Innerhalb 5% vom 52W-Hoch (echter Ausbruch möglich)
    "ema200_above":            10,   # Über EMA200 (Langfristtrend intakt)
    "adx_trending":            10,   # ADX > 25 (echter Trend, kein Seitwärts)
}
...
COMBO_BONUSES = [
    ...
    (["ma60_crossover_recent", "higher_highs_lows", "volume_on_up_day"],    15, "📈 CONFIRMED BREAK"),
...
def analyze(ticker: str, df: pd.DataFrame) -> dict | None:
    try:
        ...
        # Max erreichbarer Score inkl. aller Kombis
        all_combos    = sum(b for _, b, _ in COMBO_BONUSES)
        total_max     = base_max + all_combos
        breakout_prob = round(min(score / total_max * 100, 99), 1)
...
def build_html(df: pd.DataFrame) -> str:
    top     = df.head(150)
    MAX_TOT = int(df["total_max"].iloc[0]) if len(df) > 0 else 500
    ...
    for i, row in top.iterrows():
        ...
        <tr{border_s} class="data-row"
          ...
          <td style="padding:10px 14px;font-weight:800;color:#fff;font-size:13px">{int(row['score'])}<span style="color:#333;font-size:10px">/{MAX_TOT}</span></td>
        </tr>"""
...
<title>Breakout Screener v2.0 · Russell 2000</title>
...
<div class="wrap">
  <header>
    ...
    <h1>Breakout <em>Intelligence</em> <span class="version">v2.0</span></h1>
  ...
  <div class="stats">
    ...
    <div class="stat"><div class="slabel">Max Score</div><div class="sval white">{MAX_TOT}</div></div>
  ...
  <div class="footer">
    // Russell 2000 Advanced Breakout Screener v2.0 · Daten: Yahoo Finance via yfinance<br>
    // Vorfilter aktiv · Kombinations-Boni · Nicht als Anlageberatung zu verstehen
  </div>
</div>
<script>
let curF = 'all';
...
def main():
    print("╔" + "═"*60 + "╗")
    print("║  ADVANCED BREAKOUT SCREENER v2.0 · RUSSELL 2000" + " "*11 + "║")
    print("║  Vorfilter + Kombinations-Boni + Qualitäts-Scoring" + " "*8 + "║")
    print("╚" + "═"*60 + "╝")
    ...
    csv_cols = ["ticker", "signal", "category", "score", "combo_score", "active_combos",
                "breakout_prob", "price", "ma60", "pct_from_ma60", "rsi", "macd_hist",
    ...
    # Konsolen-Ausgabe
    print(f"\n{'═'*80}")
    print(f"  🏆 TOP {TOP_N_CONSOLE} BREAKOUT-KANDIDATEN")
    print(f"{'═'*80}")
    print(f"  {'TICKER':<8} {'SIGNAL':<16} {'PROB':>5} {'SCORE':>6}  {'KURS':>7}  "
          f"{'RSI':>5}  {'VOL×':>5}  KOMBOS")
    print(f"  {'─'*8} {'─'*16} {'─'*5} {'─'*6}  {'─'*7}  {'─'*5}  {'─'*5}  {'─'*25}")
    ...
    for _, r in df_results.head(TOP_N_CONSOLE).iterrows():
        ...
        print(f"  {r['ticker']:<8} {sig_s:<16} {r['breakout_prob']:>4.0f}% "
              f"{int(r['score']):>5}/{r['total_max']}  "
              f"${r['price']:>6.2f}  {r['rsi']:>5.0f}  "
              f"{r['vol_ratio']:>4.1f}×  {cat_icon} {combos}")

    print(f"\n  ℹ  Vorfilter: min ${MIN_PRICE}, min Vol {MIN_AVG_VOLUME:,}, "
          f"max {MAX_DIST_BELOW_MA60*100:.0f}% unter MA60, 20T-Trend > {MIN_PRICE_TREND_20D*100:.0f}%")
    print(f"  ℹ  Basis: {sum(WEIGHTS.values())} Pkt · "
          f"+ bis zu {sum(b for _,b,_ in COMBO_BONUSES)} Kombo-Bonus-Pkt\n")
