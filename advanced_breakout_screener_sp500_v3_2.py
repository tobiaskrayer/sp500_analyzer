...
def analyze_sp500_health() -> dict:
    ...
    try:
        ...
        # ── GESAMT-SCORE & URTEIL (gewichtet) ─────────────────────────────
        bull_weighted = sum(w for w, _ in bull)
        bear_weighted = sum(w for w, _ in bear)
        total_weight  = bull_weighted + bear_weighted
        if total_weight > 0:
            raw_score = 50 + (bull_weighted - bear_weighted) / total_weight * 50
...
# NACHRICHTEN-ANALYSE
# ──────────────────────────────────────────────
NEWS_TOP_N = 10    # Für wie viele Top-Aktien Nachrichten laden
...
def build_html(df: pd.DataFrame, spx: dict = None, fgi: dict = None,
    ...
    for i, row in top.iterrows():
        ...
        if has_news:
            ...
            for h in headlines[:4]:
                ...
                title_part = (
                    ...
                    f'style="color:#ccc;text-decoration:none;border-bottom:1px solid #2a2a3a" '
                    f'onmouseover="this.style.color=\'#00ff88\';this.style.borderBottomColor=\'#00ff88\'" '
                    f'onmouseout="this.style.color=\'#ccc\';this.style.borderBottomColor=\'#2a2a3a\'">'
                    f'{h["title"]}</a>'
                    if url else h["title"]
                )
                hl_html += (f'<div style="display:flex;gap:8px;margin-bottom:6px;align-items:baseline">'
                            ...
                            f'<span style="color:#555;margin-left:8px;font-size:10px">'
            ...
            if sec_files:
                ...
                for f in sec_files[:5]:
                    ...
                    sec_items += (f'<div style="display:flex;gap:8px;margin-bottom:5px;align-items:baseline">'
                                  ...
                                  f'<span style="color:#555;margin-left:8px;font-size:10px">{fdate}</span>'
                                  f'</span></div>')
                sec_html = f"""
              <div style="border-top:1px solid #1a1a28;padding-top:10px;margin-top:4px;margin-bottom:10px">
                <div style="font-size:9px;color:#555;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px">
            ...
            news_expand = f"""
        <tr id="news-{ticker}" style="display:none;background:#0a0a12">
          <td colspan="{col_count}" style="padding:0">
            <div style="padding:16px 20px 16px 24px;border-left:3px solid {rec_color};
                        border-bottom:1px solid #1a1a28;font-family:'Courier New',monospace">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                <div style="font-size:10px;color:#777;letter-spacing:2px;text-transform:uppercase">
                  📰 Nachrichten · {n_art} Artikel · Yahoo Finance + Google News
                </div>
              ...
              {sec_html}
              <div style="font-size:11px;color:#777;border-top:1px solid #1a1a28;padding-top:10px">
                💡 {reason}
              </div>
            </div>
          </td>
        ...
        <tr{border_s} class="data-row"
            ...
            style="{'border-left:3px solid #00ff88;' if has_border else ''}{row_cursor}"{row_onclick}{row_title}>
          <td style="padding:10px 14px;font-weight:900;font-size:14px;font-family:'Courier New',monospace">
            ...
            <a href="https://finance.yahoo.com/quote/{ticker}" target="_blank"
               style="color:#fff;text-decoration:none;border-bottom:1px solid #333;padding-bottom:1px"
               onmouseover="this.style.color='#00ff88';this.style.borderBottomColor='#00ff88'"
               onmouseout="this.style.color='#fff';this.style.borderBottomColor='#333'"
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .news-card{{background:#0f0f18;border:1px solid #1a1a28;border-radius:8px;padding:18px 20px}}
  .news-card:hover{{border-color:#333}}
  .news-ticker{{font-size:18px;font-weight:900;color:#fff;font-family:'Courier New',monospace}}
  ...
  .data-row[onclick]:hover{{background:#0d0d1a}}
  .news-arrow{{font-size:10px;color:#444;margin-right:4px;display:inline-block;transition:transform .2s}}
