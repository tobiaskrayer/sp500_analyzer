...
# ── HTTP-RETRY-HELPER ────────────────────────────────────────────────────────
def _requests_get_retry(url: str, timeout: int = 12, headers: dict = None,
                        max_retries: int = 2, backoff: float = 1.0) -> requests.Response | None:
    """GET-Request mit einfachem Retry und exponentiellem Backoff."""
    for attempt in range(max_retries + 1):
        try:
            resp = requests.get(url, timeout=timeout, headers=headers or {})
            if resp.status_code == 200:
                return resp
            # Bei 429 (Rate Limit) oder 5xx: Retry
            if resp.status_code in (429, 500, 502, 503, 504) and attempt < max_retries:
                time.sleep(backoff * (2 ** attempt))
                continue
            return resp  # Anderer Fehlercode → direkt zurück
...
# Sortierte Phrasen-Listen für Longest-Match-First
# Längere Phrasen werden zuerst geprüft, damit "raised guidance" nicht als
# separates "raised" + unbekanntes "guidance" gematcht wird und Sätze wie
# "strong headwinds" nicht fälschlich als bullish gewertet werden.
_BULLISH_SORTED = sorted(BULLISH_WORDS.items(), key=lambda x: -len(x[0]))
_BEARISH_SORTED = sorted(BEARISH_WORDS.items(), key=lambda x: -len(x[0]))


def _score_text(text: str) -> float:
    """
    Gibt einen Sentiment-Score zurück: positiv > 0, negativ < 0.
    Verwendet Longest-Match-First mit Phrasen-Maskierung:
    Bereits gematchte Textbereiche werden nicht erneut gescort.
    """
    text_lower = text.lower()
    score = 0.0
    # Maske: True = bereits von einer Phrase abgedeckt
    masked = [False] * len(text_lower)

    def _is_masked(start: int, end: int) -> bool:
        return any(masked[i] for i in range(start, end))

    def _mark_masked(start: int, end: int):
        for i in range(start, end):
            masked[i] = True

    # Bullish zuerst (längste Phrasen zuerst)
    for phrase, weight in _BULLISH_SORTED:
        idx = text_lower.find(phrase)
        while idx != -1:
            end = idx + len(phrase)
            if not _is_masked(idx, end):
                score += weight
                _mark_masked(idx, end)
            idx = text_lower.find(phrase, end)

    # Bearish (längste Phrasen zuerst)
    for phrase, weight in _BEARISH_SORTED:
        idx = text_lower.find(phrase)
        while idx != -1:
            end = idx + len(phrase)
            if not _is_masked(idx, end):
...
def _fetch_rss_news(ticker: str) -> list[dict]:
    ...
    try:
        ...
        resp = _requests_get_retry(url, timeout=12, headers={"User-Agent": "Mozilla/5.0"})
...
# ── SEC EDGAR ────────────────────────────────────────────────────────────────
# SEC EDGAR verlangt eine gültige Kontakt-E-Mail im User-Agent.
# Bitte anpassen, da sec.gov mit "example.com" Rate-Limits verhängen kann.
SEC_USER_AGENT = "BreakoutScreener/3.1 (kontakt@deine-email.de)"

# Globaler CIK-Cache damit nicht bei jedem Ticker neu geladen werden muss
_cik_cache: dict = {}
_cik_lock = threading.Lock()  # Thread-Safety für den globalen Cache

def _get_cik(ticker: str) -> str | None:
    """Gibt die SEC CIK-Nummer für einen Ticker zurück (thread-safe mit Cache)."""
    global _cik_cache
    with _cik_lock:
        if not _cik_cache:
            try:
                resp = _requests_get_retry(
...
def analyze_ticker_news(ticker: str, row: dict) -> dict:
    ...
    # Wenn yfinance/RSS nichts geliefert haben, ist Google News der alleinige Fallback.
